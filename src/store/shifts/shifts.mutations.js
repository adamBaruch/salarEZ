export default {
    ...
    deleteShift: ((state, {id, date}) => {
        const index = state.shifts[date.year][date.month].findIndex(temp => temp.id === id);
        const deleted = state.shifts[date.year][date.month].splice(index, 1);
